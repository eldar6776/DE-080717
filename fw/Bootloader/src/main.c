...
void main(void)
{
    ...
    else // if ((PORTBbits.RB7 == 0) || (PORTBbits.RB6 == 0))
    {
...
void InitializeSystem(void)
{
    ...
    *((unsigned char*) 0xFB5) = 0x90; //Enable active clock tuning for USB operation)
    ...
    RedLed_Off(); //LED off initially
    TRISAbits.TRISA7 = 0; //Configure pin as output
#endif
...
void BlinkUSBStatus(void)
{
    ...
    if (led_count == 0)
    {
        ...
        {
            RedLed_On(); //Turn on the LED continuously
        }
        else
        {
            RedLed_Toggle(); //Toggle the LED state
        }
    }
